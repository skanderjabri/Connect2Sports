...
import GetAllEmsSalleApi from "../api/GetAllEmsSalleApi";
const { width } = Dimensions.get("window");
const CARD_WIDTH = width * 0.9;


export default function SallesEms() {
    ...
    const fetchSallePadel = async (isLoadMore = false) => {
        try {
            const response = await GetAllEmsSalleApi(isLoadMore ? page + 1 : 1, limit);
            if (isLoadMore) {
                if (response.emsSalles.length > 0) {
                    setSallePadels((prev) => [...prev, ...response.emsSalles]);
                    setFilteredPadels((prev) => [...prev, ...response.emsSalles]); // Mettre à jour les données filtrées
                    setPage(page + 1);
                } else {
                    setHasMore(false);
                }
            } else {
                setSallePadels(response.emsSalles);
                setFilteredPadels(response.emsSalles);
                setPage(1);
                setHasMore(true);
            }
        } catch (error) {
    ...
    const renderPadelCard = ({ item }) => (
        <View style={styles.cardContainer} key={item._id}>
            <TouchableOpacity
                activeOpacity={0.95}
                onPress={() => router.push(`/DetailsSalleEms/${item._id}`)}
            >
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.headerContainer}>
                ...
                <Text style={styles.headerTitle}>Salles EMS</Text>
            </View>
            {/* Champ de recherche */}
