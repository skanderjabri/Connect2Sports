...
import GetAllTerrainFootBallApi from '../api/GetAllTerrainFootBallApi'
...
export default function FootballTerrainsScreen() {
    ...
    const fetchSallePadel = async (isLoadMore = false) => {
        try {
            const response = await GetAllTerrainFootBallApi(isLoadMore ? page + 1 : 1, limit);
            if (isLoadMore) {
                if (response.footballTerrains.length > 0) {
                    setSallePadels((prev) => [...prev, ...response.footballTerrains]);
                    setFilteredPadels((prev) => [...prev, ...response.footballTerrains]); // Mettre à jour les données filtrées
                    setPage(page + 1);
                } else {
                    setHasMore(false);
                }
            } else {
                setSallePadels(response.footballTerrains);
                setFilteredPadels(response.footballTerrains); // Initialiser les données filtrées
                setPage(1);
                setHasMore(true);
            }
        } catch (error) {
    ...
    const renderPadelCard = ({ item }) => (
        <View style={styles.cardContainer} key={item._id}>
            <TouchableOpacity
                activeOpacity={0.95}
                onPress={() => router.push(`/TerrainFootBall/${item._id}`)}
            >
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.headerContainer}>
                ...
                <Text style={styles.headerTitle}>Terrains de Football</Text>
            </View>
            {/* Champ de recherche */}
