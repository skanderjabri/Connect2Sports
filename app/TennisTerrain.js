...
import GetTennisTerrainApi from "../api/GetTennisTerrainApi";
...
export default function TennisTerrain() {
    ...
    const fetchSallePadel = async (isLoadMore = false) => {
        try {
            const response = await GetTennisTerrainApi(isLoadMore ? page + 1 : 1, limit);
            if (isLoadMore) {
                if (response.tennisTerrains.length > 0) {
                    setSallePadels((prev) => [...prev, ...response.tennisTerrains]);
                    setFilteredPadels((prev) => [...prev, ...response.tennisTerrains]); // Mettre à jour les données filtrées
                    setPage(page + 1);
                } else {
                    setHasMore(false);
                }
            } else {
                setSallePadels(response.tennisTerrains);
                setFilteredPadels(response.tennisTerrains); // Initialiser les données filtrées
                setPage(1);
                setHasMore(true);
            }
        } catch (error) {
    ...
    const renderPadelCard = ({ item }) => (
        <View style={styles.cardContainer} key={item._id}>
            <TouchableOpacity
                activeOpacity={0.95}
                onPress={() => router.push(`/TerrainTennis/${item._id}`)}
            >
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.headerContainer}>
                ...
                <Text style={styles.headerTitle}>Courts de Tennis</Text>
            </View>
            {/* Champ de recherche */}
