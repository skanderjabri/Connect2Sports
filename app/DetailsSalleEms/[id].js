...
import GetEmsSalleByIdApi from '../../api/GetEmsSalleByIdApi'
const { width } = Dimensions.get('window');
const SPACING = 15;

export default function DetailsSalleEms() {
    ...
    const fetchSalle = async () => {
        try {
            const response = await GetEmsSalleByIdApi(id);
            setTerrainPadel(response.salle);
        } catch (error) {
